...
#
# Create an image that can by written into a SPI NAND flash technology using dd application.
...
IMAGE_CMD_sunxi-spinand () {

	# Initialize spi nand flash image file
	...
	dd if=${DEPLOY_DIR_IMAGE}/${SPL_BINARY} of=${SPINAND} bs=1k conv=notrunc
	dd if=${DEPLOY_DIR_IMAGE}/${KERNEL_DEVICETREE} of=${SPINAND} bs=1k seek=448 conv=notrunc
	dd if=${DEPLOY_DIR_IMAGE}/${KERNEL_IMAGETYPE} of=${SPINAND} bs=1k seek=512 conv=notrunc
	dd if=${DEPLOY_DIR_IMAGE}/jffs2.img of=${SPINAND} bs=1k seek=4864 conv=notrunc
